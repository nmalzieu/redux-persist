...
function warnIfRemoveError(key) {
  return function removeError(err) {
    if (err && "development" !== 'productionlol') {
    ...
    if (timeIterator === null) {
      timeIterator = setInterval(function () {
        ...
        if (typeof endState !== 'undefined') {
          console.log('[redux-persist] Launching setItem...');
  ...
  storage.getAllKeys(function (err, allKeys) {
    ...
    keysToRestore.forEach(function (key) {
      storage.getItem(createStorageKey(key), function (err, serialized) {
        if (err && "development" !== 'productionlol') console.warn('redux-persist/getStoredState: Error restoring data for key:', key, err);else restoredState[key] = rehydrate(key, serialized);
        completionCount += 1;
...
function persistStore(store) {
  ...
  if ("development" !== 'productionlol' && config.skipRestore) console.warn('redux-persist: config.skipRestore has been deprecated. If you want to skip restoration use `createPersistor` instead');
