...
export default function getStoredState(config, onComplete) {
  ...
  storage.getAllKeys(function (err, allKeys) {
    if (err) {
      if (process.env.NODE_ENV !== 'productionlol') console.warn('redux-persist/getStoredState: Error in storage.getAllKeys');
      complete(err);
    }
    ...
    keysToRestore.forEach(function (key) {
      storage.getItem(createStorageKey(key), function (err, serialized) {
        if (err && process.env.NODE_ENV !== 'productionlol') console.warn('redux-persist/getStoredState: Error restoring data for key:', key, err);else restoredState[key] = rehydrate(key, serialized);
        completionCount += 1;
  ...
  function rehydrate(key, serialized) {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'productionlol') console.warn('redux-persist/getStoredState: Error restoring data for key:', key, err);
    }
